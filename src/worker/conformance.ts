...
const E = {
  ...
  change: ev("Controlled change procedure", "/controls/#secure-development"),
  release: ev("Current release identity", "/version.json"),
...
export const CHANGE_PROCESSES: readonly ChangeProcess[] = [
  {
    ...
    status: "met",
    steps: [
      "Classify the change with the current WG-ARCH-001 structured type enforced by the repository contract.",
    ...
    records: ["Change identifier and label on the Git/GitHub history", "Release identity", "Classification record in DOC-27, naming the class assigned to every change and what the class decides"],
    clauses: ["27001 Clause 6.3", "27001 Clause 8.1", "27001 A.8.32", "42001 Clause 6.3"],
    evidence: [E.change, E.release, E.docRecords],
  },
  {
    id: "CM-02",
    title: "Change request and Git/GitHub history",
    ...
    status: "met",
    steps: [
      "Open a controlled change with the next sequential ST-### identifier.",
    ...
    records: ["Published controlled Git change with identifier, title, summary, evidence list and reference"],
    clauses: ["27001 A.8.32", "27001 A.5.37", "42001 A.6.2.3"],
    evidence: [E.change, E.release],
  },
  {
    id: "CM-03",
    ...
    records: ["Control receipt with sequence, code, decision, outcome, timestamp and hash", "Authorisation record in DOC-27 — the release identity carried by every controlled Git change, which is the authorisation and the change as one record"],
  ...
  },
  {
    ...
    records: ["Gate output retained by the operator", "Post-deploy route checks recorded in the controlled Git change evidence"],
  ...
  },
  {
    ...
    steps: [
      ...
      "Publish the controlled Git change with its closure time and the evidence it produced, marked as a hotfix.",
    ],
    records: ["Hotfix controlled Git change with closure time and reference", "Incident and receipt where service was affected"],
  ...
  },
  {
    ...
    steps: [
      ...
      "Confirm the evidence items claimed by the controlled Git change are actually observable.",
    ...
    records: ["Evidence list per controlled Git change", "Incident closure state", "Post-implementation review record in DOC-27, sampled against the evidence bullets written after each change shipped"],
  ...
  },
  {
    ...
    records: ["Incident entry", "Controlled Git change where a tolerance was added", "Unintended-change review record in DOC-27, across four independent signals", "Daily state copy under a digest, making an unintended change to durable state detectable by comparison"],
  ...
  },
  {
    ...
    records: ["Deterministic tank log", "Replay at a chosen tick", "Controlled Git change"],
...
export const MANDATORY_DOCUMENTS: readonly DocumentItem[] = [
  ...
  { ref: "DOC-11", title: "Operational planning and control records", clause: "27001 Clause 8.1", status: "met", note: "Every production change is recorded with an identifier, a release identity and its evidence; control actions are receipted.", evidence: [E.change, E.receipts] },
  ...
  { ref: "DOC-13", title: "Results of information security risk treatment", clause: "27001 Clause 8.3", status: "met", note: "Each treatment is a published controlled Git change naming the finding it closes, the fix, and the evidence produced, and the treatment plan records what is in place and what residual risk the Owner is accepting on each.", evidence: [E.docPlan, E.change, E.release] },
...
const ISO27001_CLAUSES: Control[] = [
  ...
  { ref: "4.4", title: "Information security management system", ask: "Show that the ISMS and its processes are established, implemented, maintained and continually improved.", status: "met", note: "The management system is stated as a set of processes with their interactions — risk assessment feeds the treatment plan, the plan produces changes, controlled changes are retained in Git/GitHub, operating them produces runtime records, and the register cites those records. The runtime policy and register render from tracked source shipped with the service; Git/GitHub remains the separate authority for implementation history.", evidence: [E.docPlanning, E.register, E.change] },
  ...
  { ref: "8.3", title: "Information security risk treatment (performance)", ask: "Show the treatment plan implemented and its documented results.", status: "met", note: "Every finding closed to date is published as a controlled Git change naming the finding, the fix, the closure time and the evidence produced, under a treatment process that states how the decision between treat, accept, avoid and share is taken.", evidence: [E.docRiskTreatment, E.docPlan, E.change, E.release] },
  ...
  { ref: "10.1", title: "Continual improvement", ask: "Show that the suitability, adequacy and effectiveness of the ISMS is continually improved.", status: "met", note: "The mechanism is that the register keeps current shortfalls public while Git/GitHub retains the changes that address them. Runtime evidence shows the operating controls themselves, and open gaps remain explicit rather than being hidden inside implementation chronology.", evidence: [E.docPlanning, E.change, E.register] },
...
const ISO27001_ANNEX_A: Control[] = [
  ...
  { ref: "A.8.32", title: "Change management", ask: "Show changes subject to change management procedures.", status: "met", note: "Fourteen documented processes cover classification, request, authorisation, environment separation, verification, release, configuration, rollback, emergency change, post-implementation review, unintended change, supplier change, record integrity and AI change. Every production change carries an identifier, a release identity and its evidence, and the record is public.", evidence: [E.register, E.change, E.release] },
...
const ISO42001_CLAUSES: Control[] = [
  ...
  { ref: "10.1", title: "Continual improvement", ask: "Show continual improvement of the AI management system.", status: "met", note: "Improved by the same controlled-change mechanism and retained in Git/GitHub. AI-system monitoring is tied to explicit objectives and is reviewed against the running configuration and source rather than against an implementation changelog.", evidence: [E.docPlanning, E.docObjectives, E.change] },
...
export const EVIDENCE_INDEX: readonly EvidenceRoute[] = [
  ...
  { route: "/controls/#secure-development", access: "public", proves: "The current controlled-change, verification, release, and rollback procedure; Git/GitHub is authoritative for implementation history.", controls: "8.1, 8.3, A.8.32, A.5.27" },
  { route: "/version.json", access: "public", proves: "The semantic release identity served by the running product without reproducing implementation chronology.", controls: "8.1, A.8.32" },
...
export function conformanceHtml(metricCard: MetricCard, embedded = false): string {
  ...
  <div class="card">
    ...
    <p class="sub u-m-14-0-0">Operational controls for the running service remain protected behind operations credentials. Their public records are consolidated under <a href="/evidence/#incidents">Incidents</a>, <a href="/evidence/#logs">Logs</a>, and <a href="/controls/#secure-development">Change management</a>.</p>
  </div>
  ${filterScript()}`;
}
