...
export const MANDATORY_DOCUMENTS: readonly DocumentItem[] = [
  ...
  { ref: "DOC-07", title: "Risk treatment plan", clause: "27001 Clause 6.1.3 e)", status: "met", note: "Issued: twelve assessed risks, each scored under the stated method with the decision taken, what is in place, and what is left. Nine are treated or accepted with the residual named; three stay open — no backup of the object holding the receipt chain, no dependency scanning, and no route by which a player can have their profile erased.", evidence: [E.docPlan, E.policiesJson] },
...
const ISO27001_CLAUSES: Control[] = [
  ...
  { ref: "5.1", title: "Leadership and commitment", ask: "Show how top management demonstrates commitment: resources, direction, integration into business processes.", status: "partial", note: "Commitment is evidenced by the policy set and by the spend ceiling being enforced rather than negotiable. Direction and resourcing of persons cannot be demonstrated with one person, so this stays short of met.", evidence: [E.docSecurity, E.docRoles] },
  ...
  { ref: "7.3", title: "Awareness", ask: "Show that people are aware of the policy, their contribution, and the consequences of not conforming.", status: "gap", note: "The policy set now exists to be aware of, but no awareness activity has been performed or recorded against it. With one person the clause is close to vacuous; it stays a gap rather than being written away.", evidence: [ev("Awareness record required")] },
...
const ISO27001_ANNEX_A: Control[] = [
  ...
  { ref: "A.5.3", title: "Segregation of duties", ask: "Show conflicting duties separated, or the compensating controls where they cannot be.", status: "partial", note: "Segregation of duties is impossible with one person and the policy set says so plainly. The compensating design is detection rather than separation: control actions write receipts into an anchored hash chain, and deployments appear in the public change record.", evidence: [E.docRoles, E.receipts, E.change] },
  { ref: "A.5.4", title: "Management responsibilities", ask: "Show management requiring personnel to apply security per policy.", status: "partial", note: "Responsibilities are documented, but a management responsibility to ensure personnel apply them has no meaning with a single operator. Recorded as a limitation rather than claimed.", evidence: [E.docRoles] },
  ...
  { ref: "A.5.22", title: "Monitoring, review and change management of supplier services", ask: "Show regular monitoring and review of supplier service delivery and changes.", status: "partial", note: "Monitoring is continuous and published: consumption per bound service, availability from project start, and provider-side outage appearing as unscheduled downtime in this service's own record rather than being taken from a status page. The review cycle is defined — the runtime pin and the provider's published changes, every 90 days — but no cycle has completed, because this is the deployment that defines it. Met on the first review that produces a record. See CM-12.", evidence: [E.docAccess, E.status, E.inquiry] },
  ...
  { ref: "A.5.36", title: "Compliance with policies, rules and standards", ask: "Show regular review of compliance with the organisation's own policies.", status: "partial", note: "The rule is stated — a control recorded as met must name a live route, and anything unevidenced is recorded as a gap — and the register applies it. What is missing is a dated compliance review producing a record of its own.", evidence: [E.docSecurity, E.register] },
  ...
  { ref: "A.6.7", title: "Remote working", ask: "Show security measures for working outside the organisation's premises.", status: "partial", note: "All work is remote and there are no premises. The rules now exist: one unshared machine, no deployment from another machine or over an uncontrolled network, and credentials never written into tracked files. The parts that live in the repository can be checked — the ignore rules, the account identifier absent from tracked configuration, the deploy script refusing without credentials supplied from outside it. The rest is attested about a machine no route can observe, so this stays short of met.", evidence: [E.docAccess, E.docSecureDev] },
  ...
  { ref: "A.7.7", title: "Clear desk and clear screen", ask: "Show clear desk and clear screen rules defined and enforced.", status: "partial", note: "The clear desk half is recorded as inapplicable and why: no desk, no paper, no printer, no removable media and no office. The clear screen half applies and the rule is stated — the machine is locked when left and the control panel is not left open unattended, with the note that an open tab exposes what is displayed rather than the credential, since re-authorisation is a browser prompt. Defined, but enforcement is attested rather than evidenced.", evidence: [E.docAccess] },
  ...
  { ref: "A.7.9", title: "Security of assets off-premises", ask: "Show protection of assets used away from premises.", status: "partial", note: "The operator machine is the only asset off premises, since every premises is off premises here. It is now covered by documented rules, and by a compensating position that is provable: endpoint compromise is detectable rather than preventable, because every production change appears in the public change record and every control action writes a receipt into the anchored chain. An attacker with the machine could do damage; they could not do it quietly.", evidence: [E.docAccess, E.change, E.receipts] },
  ...
  { ref: "A.8.1", title: "User endpoint devices", ask: "Show information on endpoint devices protected.", status: "partial", note: "One endpoint exists and its rules are documented. What is provable from the repository: credentials are never in tracked files, the account identifier is deliberately absent from the tracked configuration, both credential filenames are excluded from version control so committing one is an error rather than an oversight, and the production operations secrets are platform secrets rather than files the machine holds in a usable form. The device's own state is attested, not evidenced, so this is partial by the honesty rule rather than met.", evidence: [E.docAccess, E.docSecureDev] },
...
const ISO42001_CLAUSES: Control[] = [
  ...
  { ref: "4.4", title: "AI management system", ask: "Show the AIMS established, implemented, maintained and improved.", status: "partial", note: "The AI policy, roles and impact assessment are established and published. The management system around them — audit, review, improvement cycle — is not yet running, so this is short of met.", evidence: [E.docAi, E.policies] },
  ...
  { ref: "7.3", title: "Awareness", ask: "Show awareness of the AI policy and of individual contribution to AIMS effectiveness.", status: "gap", note: "An AI policy now exists to be aware of, but no awareness activity has been performed or recorded against it.", evidence: [ev("Awareness record required")] },
...
const ISO42001_ANNEX_A: Control[] = [
  ...
  { ref: "A.2.4", title: "Review of the AI policy", ask: "Show the AI policy reviewed at planned intervals.", status: "partial", note: "A review trigger is stated — before any change to how sharks decide, and at least every 90 days — but no review has yet been performed, so there is no review record to show.", evidence: [E.docAi] },
  ...
  { ref: "A.4.6", title: "Human resources", ask: "Show the human resources for the AI system, including competences.", status: "partial", note: "The human resource is one person, stated as such. Competence is claimed from the work product rather than from held qualifications, which is weaker than the clause envisages.", evidence: [E.docRoles] },
...
export const EVIDENCE_INDEX: readonly EvidenceRoute[] = [
  ...
  { route: "/policies/ and /policies.json", access: "public", proves: "The written record the standards ask for: scope and context, the information security policy, roles and authorities, the risk assessment method and its acceptance criteria, the treatment process, the Statement of Applicability cover, the assessed risks with their scores and decisions, the objectives, the AI policy with its impact assessment, the AI system life cycle, the secure development and operations procedure, the access control, supplier and endpoint policy, and the legal, regulatory and contractual register with its privacy notice — each naming the clauses it is the record for.", controls: "27001 4.1-4.3, 5.1-5.3, 6.1.1-6.1.3, 6.2, 8.2, 8.3, A.5.1-A.5.4, A.5.8, A.5.10, A.5.15, A.5.19-A.5.23, A.5.31, A.5.32, A.5.34, A.5.36, A.5.37, A.6.7, A.7.7, A.7.9, A.8.1, A.8.7, A.8.24, A.8.25, A.8.28, A.8.29; 42001 4.1-4.4, 5.2-5.3, 6.1.2-6.1.4, 6.2, 8.2-8.4, A.2, A.3.2, A.4.2, A.4.4, A.5, A.6.1.2, A.6.2, A.7.4, A.8.2, A.8.5, A.9.2-A.9.4, A.10.4" },
