...
/** Development minutes across every post-delivery hotfix, WG-018 to WG-039. */
...
function inquiryHtml(billing: Record<string, unknown>): string {
  ...
  return `<section class="page-intro"><div class="eyebrow">Shark Tank cost control</div><h1>Every bite leaves a receipt.</h1><a class="action-link" href="/inquiry.json">Raw inquiry JSON →</a></section>
    ...
    <div class="card"><h2>Usage against the free tier</h2>
      ...
      <div class="table-scroll" role="region" aria-label="Usage against the free tier" tabindex="0"><table class="billing-table meter-table"><thead><tr><th>Service</th><th>Used to date</th><th>Limit</th><th>Today${todayPartial ? "*" : ""}</th><th>Daily average</th></tr></thead><tbody>
...
function incidentTimelineSvg(incidents: IncidentRecord[], now = Date.now(), history: ControlHistoryEntry[] = [], linkBase = ""): string {
  ...
  return `<div class="timeline-scroll" role="region" aria-label="Availability timeline" tabindex="0"><svg role="img" aria-label="${esc(`Server and tank availability from project start to now, ${formatWindow(span)} measured`)}" viewBox="0 0 ${width} 90" preserveAspectRatio="xMidYMid meet">
...
function controlHistoryListHtml(history: ControlHistoryEntry[], integrity: ControlHistoryIntegrity): string {
  ...
  const head = integrity.headHash ? `<code>${esc(integrity.headHash)}</code>` : "No receipt yet";
  return `<section class="card" id="control-history"><div class="eyebrow">Control receipts</div><h2 style="margin-top:0">Append-only control history</h2><div class="integrity-line"><span class="integrity-badge">${esc(integrity.algorithm)}</span><span>${integrity.entryCount} entries · chain head ${head}</span></div><div class="integrity-line">${integrityVerdict(integrity)}</div>
...
if(i>=0){page=Math.floor(i/PER);render(0);target.classList.add('history-item--focus');target.scrollIntoView({block:'center'});}}
...
function incidentsHtml(incidents: IncidentRecord[], history: ControlHistoryEntry[], integrity: ControlHistoryIntegrity): string {
  ...
  const card = (x: IncidentRecord) => {
    ...
    return `<article class="card incident-card${x.status === "active" ? " incident-card--active" : ""}" id="${incidentAnchor(x)}"><div class="m ${x.status === "active" ? "o" : "g"}"><i class="incident-dot ${tone.key}"></i>${x.status.toUpperCase()} · ${esc(x.cause)}</div><h2>${esc(x.title)}</h2><p>${esc(x.summary)}</p><p class="sub" style="margin:0">${timing}</p>${receipt}</article>`;
  };
...
function logToolbar(tableId: string, records: Array<{ reasonCode: string }>): string {
  const pages = Math.max(1, Math.ceil(records.length / LOG_PAGE_SIZE));
  return `<div class="log-toolbar"><label><span>Search</span><input type="search" data-log-search="${tableId}" placeholder="Action, subject, detail" autocomplete="off"></label><label><span>Reason code</span><select data-log-reason="${tableId}"><option value="">All codes</option>${reasonOptions(records)}</select></label><span class="log-visible-count" id="${tableId}-count" role="status" aria-live="polite" aria-atomic="true">${esc(logCountText(records.length, records.length, 1, pages))}</span></div>`;
}
...
function publicLogsHtml(events: PublicLogEvent[], gameLogs: PublicTankLog[], caps: { serviceTruncated: boolean; captureTruncated: boolean }): string {
  ...
  return `<section class="page-intro"><div class="eyebrow">Public Shark Tank inquiry</div><h1>Every operational move leaves a reason.</h1><p class="sub">Service evidence is retained for 90 days; tank captures for 24 hours. Both are shown in full below — every row carries a reason code.</p><a class="action-link" href="/logs.json">Public log JSON →</a></section>
    <details class="card log-room"><summary><span class="log-summary"><strong>Service evidence</strong><code>90-day retention</code><span class="log-count">${serviceRecords.length} records</span></span></summary><div class="log-room-body">${logToolbar(serviceTableId, serviceRecords)}<div class="table-scroll" role="region" aria-label="Service evidence" tabindex="0"><table class="events-table" id="${serviceTableId}"><thead><tr><th>Timestamp</th><th>Reason</th><th>Action</th><th>Subject</th><th>Detail</th></tr></thead><tbody>${rows || '<tr><td colspan="5">No public events recorded.</td></tr>'}</tbody></table></div>${serviceRecords.length > LOG_PAGE_SIZE ? logPager(serviceTableId, serviceRecords.length) : ""}<p class="table-note">Reason codes are one letter plus three digits. Full detail remains available in JSON.</p></div></details>
          ...
          shell(
            ...
            `<section class="page-intro"><div class="eyebrow">Tank Access</div><h1>Availability status</h1><a class="action-link" href="/status.json">Raw status JSON →</a></section>
             ...
             <div class="card"><h2 style="margin-top:0;font-size:1.1rem">Tank activity</h2>
               <div class="table-scroll" role="region" aria-label="Tank activity" tabindex="0"><table class="capacity-table"><thead><tr><th>Tank</th><th>Active players</th><th>Top score</th><th>Leader</th></tr></thead><tbody id="status-tank-rows">${roomRows}</tbody></table></div>
             </div>
             ${statusLiveScript()}`,
          ),
        );
