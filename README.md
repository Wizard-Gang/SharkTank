...
Pushing a semantic `vX.Y.Z` tag triggers the Release workflow. The workflow installs locked dependencies, runs `npm run check` plus the live dependency-advisory gate, verifies the exact annotated release identity, and publishes the GitHub Release. Only after publication succeeds may it call the reusable `.github/workflows/deploy.yml` stage with that same event tag. Production deployment remains optional behind `PRODUCTION_DEPLOY_ENABLED=true`, the protected `production` environment, Cloudflare credentials, and an independent exact-tag identity check. The real deployment command additionally refuses unless it is running in GitHub Actions for the exact SharkTank Release workflow semantic-tag context; it never loads local `.env` authority. The ignored `.env` is available only to the explicit local dry-run.
