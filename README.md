...
| `npm run local` | Whole-stack convenience lifecycle: may run `npm install` when `node_modules` is absent, stops only a previously recorded Wrangler lifecycle and a positively identified checkout-local Workerman master, and fails closed if ports 8787/8080/8081 are occupied by anything else. By default it clears only checkout-local disposable `dist/` and `.wrangler/` state and preserves PHP `packages/php-runtime/data/`. `npm run local -- --reset-php-data` is the sole destructive data opt-in and may remove only that checkout's canonical PHP data directory after containment and symlink checks. It then builds, starts PHP when available, opens a browser after a fixed delay, and runs Wrangler in the foreground. |
