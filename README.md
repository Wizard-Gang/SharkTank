...
After a merged `main` commit passes CI, the Release Tag workflow compares the root package version with its parent. Ordinary commits create no tag; an intentional lockstep `package.json`/`package-lock.json` version increase creates exactly one annotated `vX.Y.Z` tag on that accepted commit, while a matching existing tag is idempotent and any conflicting tag fails without being moved. That semantic tag then triggers the Release workflow, which installs locked dependencies, runs `npm run check` plus the live dependency-advisory gate, verifies the exact annotated release identity, and publishes the GitHub Release. Only after publication succeeds may it call the reusable `.github/workflows/deploy.yml` stage with that same event tag. Production deployment remains optional behind `PRODUCTION_DEPLOY_ENABLED=true`, the protected `production` environment, Cloudflare credentials, and an independent exact-tag identity check. The real deployment command additionally refuses unless it is running in GitHub Actions for the exact SharkTank Release workflow semantic-tag context; it never loads local `.env` authority. The ignored `.env` is available only to the explicit local dry-run.
